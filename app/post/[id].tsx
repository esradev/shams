...
const PostDetail = () => {
  ...
  useEffect(() => {
    const fetchPost = async () => {
      try {
        const response = await axios.get(`https://shams-almaarif.com/wp-json/wp/v2/posts/${id}`);
  ...
  useEffect(() => {
    const loadSound = async () => {
      if (fileUri) {
        ...
        const { sound } = await Audio.Sound.createAsync({ uri: post.meta["the-audio-of-the-lesson"] }, { shouldPlay: false });
        setSound(sound);
    ...
    <SafeAreaView className="flex-1 bg-white">
      <ScrollView className="p-4">
        <RenderHTML
          ...
          source={{
            html: post?.content.rendered || `<h2>متأسفانه هنوز متن این جلسه کامل نشده است.</h2>`
          }}
          baseStyle={{
            fontSize: 18,
            lineHeight: 26,
      ...
      {post?.meta["the-audio-of-the-lesson"] && (
        <View className="absolute bottom-0 pb-16 w-full bg-green-100 p-4 rounded-3xl align-middle ">
          <View className="flex flex-row justify-around mb-2">
            ...
            <TouchableOpacity onPress={handlePlayPause}>{isPlaying ? <MaterialIcons name="pause-circle-outline" size={30} color="black" /> : <MaterialIcons name="play-circle-outline" size={30} color="black" />}</TouchableOpacity>
          ...
          <View className="flex flex-row items-center justify-between mb-2">
            ...
            <Slider style={{ flex: 1, marginHorizontal: 8 }} minimumValue={0} maximumValue={duration} value={currentTime} onValueChange={handleSeek} minimumTrackTintColor="#16a34a" maximumTrackTintColor="gray" thumbTintColor="#16a34a" />
